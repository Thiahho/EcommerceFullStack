...
const ConsultaReparacionSection: React.FC = () => {
  ...
  const [varianteSeleccionada, setVarianteSeleccionada] =
    useState<ReparacionInfo | null>(null);
    ...
    api
      ...
      .then((res) => {
        ...
        const marcasUnicas = Array.from(
          new Set(
            (res.data as string[]).map(
              (marca: string) => marca.trim() // Solo quitar espacios, mantener formato original
            )
          )
        ).sort();
  ...
  useEffect(() => {
    if (!color && coloresDisponibles.length > 0)
      return setVarianteSeleccionada(null);
    if (!marco && marcosDisponibles.length > 0)
      return setVarianteSeleccionada(null);
    if (!version && versionesDisponibles.length > 0)
      return setVarianteSeleccionada(null);
    if (!tipo && tiposDisponibles.length > 0)
      return setVarianteSeleccionada(null);

    const variante = todosLosRegistros.find(
      (v) =>
        (coloresDisponibles.length === 0 || v.colormodulo === color) &&
        (marcosDisponibles.length === 0 ||
          (v.marco ? "Con marco" : "Sin marco") === marco) &&
  ...
  useEffect(() => {
    if (varianteSeleccionada) {
      ...
      if (varianteSeleccionada.arreglomodulo)
        servicios.push(`Módulo: $${varianteSeleccionada.arreglomodulo}`);
      if (varianteSeleccionada.arreglobat)
        servicios.push(`Batería: $${varianteSeleccionada.arreglobat}`);
      if (varianteSeleccionada.arreglopin)
        servicios.push(`Pin: $${varianteSeleccionada.arreglopin}`);
      if (varianteSeleccionada.placa)
        servicios.push(`Placa: $${varianteSeleccionada.placa}`);
  ...
  const handleEnviarWhatsApp = () => {
    ...
    if (
      !varianteSeleccionada.arreglomodulo &&
      !varianteSeleccionada.arreglobat &&
      !varianteSeleccionada.arreglopin
    ) {
                    ...
                    <div className="flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          {item.colormodulo && (
                            <span className="mr-2">
                              Color: {item.colormodulo}
                            </span>
                          )}
                          {item.tipo && (
                            <span className="mr-2">Tipo: {item.tipo}</span>
                          )}
                          {item.version && (
                            <span className="mr-2">
          ...
          <div>
            ...
            <div className="relative">
              <Input
                ...
                onBlur={() =>
                  setTimeout(() => setShowMarcaDropdown(false), 150)
                }
              />
              {showMarcaDropdown && (
          ...
          <div>
            ...
            <div className="relative">
              <Input
                ...
                onBlur={() =>
                  setTimeout(() => setShowModeloDropdown(false), 150)
                }
              />
              {showModeloDropdown && (
                <div className="absolute z-10 w-full bg-white border rounded shadow max-h-48 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-white rounded-lg p-4 border">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg">
                  {getPrecioIcon(varianteSeleccionada?.arreglomodulo)}
                </span>
              </div>
              <div
                className={`text-center py-2 px-3 rounded-md border ${getPrecioClass(varianteSeleccionada?.arreglomodulo)}`}
              >
            ...
            <div className="bg-white rounded-lg p-4 border">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg">
                  {getPrecioIcon(varianteSeleccionada?.arreglopin)}
                </span>
              </div>
              <div
                className={`text-center py-2 px-3 rounded-md border ${getPrecioClass(varianteSeleccionada?.arreglopin)}`}
              >
            ...
            <div className="bg-white rounded-lg p-4 border">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg">
                  {getPrecioIcon(varianteSeleccionada?.arreglobat)}
                </span>
              </div>
              <div
                className={`text-center py-2 px-3 rounded-md border ${getPrecioClass(varianteSeleccionada?.arreglobat)}`}
              >
          ...
          <div className="bg-white rounded-lg p-4 border border-blue-200">
            <div className="text-center">
              ...
              <p className="text-lg font-bold text-blue-600">
                {precio || "Selecciona marca y modelo"}
              </p>
            </div>
          </div>
          {loading && (
        ...
        {varianteSeleccionada && (
          <div className="bg-green-50 rounded-lg p-6">
            ...
            <p className="text-sm text-green-700 mb-4">
              Envía la cotización por WhatsApp para consultar disponibilidad y
              agendar tu reparación
            </p>
            <Button
              type="button"
              onClick={handleEnviarWhatsApp}
              className="w-full py-3 rounded-2xl bg-green-600 text-white font-bold hover:bg-green-700 transition flex items-center justify-center gap-2"
            >
              <span>📱</span>
