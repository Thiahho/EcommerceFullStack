...
const ProductDetalle: React.FC = () => {
  ...
  const rams = Array.from(
    new Set(producto.variantes.map((v: any) => v.ram))
  ) as string[];
  const almacenamientos = ram
    ? (Array.from(
        new Set(
          producto.variantes
            .filter((v: any) => v.ram === ram)
            .map((v: any) => v.almacenamiento)
        )
      ) as string[])
    : [];
  const colores =
    ram && almacenamiento
      ? (Array.from(
          new Set(
            producto.variantes
              .filter(
                (v: any) => v.ram === ram && v.almacenamiento === almacenamiento
              )
              .map((v: any) => v.color)
          )
        ) as string[])
    ...
    <div className="w-full flex flex-col items-center justify-center py-4">
      ...
      <div className="w-full max-w-3xl flex flex-col md:flex-row items-center justify-center gap-6 p-2">
        ...
        <div className="flex-1 w-full md:w-1/2 flex flex-col items-start justify-center">
          <h2 className="text-2xl font-bold mb-2 text-[#17436b]">
          ...
          {variante && (
            <div className="mb-2 w-full">
              <div className="text-lg font-bold text-green-700">
                Precio: ${variante.precio.toLocaleString()}
              </div>
          ...
          {variante && (
            <div className="w-full space-y-3">
              ...
              <button
                className="w-full bg-blue-600 text-white py-3 px-6 rounded-2xl hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-bold text-lg"
              ...
              <button
                className="w-full bg-green-600 text-white py-3 px-6 rounded-2xl hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-bold text-lg"
                disabled={!variante || variante.stock <= 0}
