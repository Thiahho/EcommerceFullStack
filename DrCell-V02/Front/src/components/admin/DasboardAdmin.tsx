...
import { BarChart3, ShoppingBag, Package, Users, Settings, TrendingUp, DollarSign, Eye, RefreshCw, AlertTriangle } from 'lucide-react';
import { dashboardService, DashboardStatsDto, ActividadRecienteDto } from '../../services/dashboardService';
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">Bienvenido al sistema de gestión integral de DrCell</p>
          </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatNumber(stats.clientesNuevos)}</div>
              <p className={`text-xs ${
                stats.cambioClientesNuevos >= 0 ? 'text-green-600' : 'text-red-600'
              }`}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatNumber(stats.productosActivos)}</div>
              <p className={`text-xs ${
                stats.cambioProductosActivos >= 0 ? 'text-green-600' : 'text-red-600'
              }`}>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatNumber(stats.alertasActivas)}</div>
              <p className={`text-xs ${
            ...
            return (
              <Card key={index} className="cursor-pointer hover:shadow-md transition-shadow">
                <CardHeader className="text-center" onClick={action.action}>
                  <div className={`mx-auto p-3 rounded-full bg-${action.color}-100 mb-3 w-fit`}>
            ...
            actividades.map((actividad, index) => {
              ...
              return (
                <div key={index} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                  <Icon className={`h-5 w-5 ${getActivityIconColor(actividad.tipo)}`} />
