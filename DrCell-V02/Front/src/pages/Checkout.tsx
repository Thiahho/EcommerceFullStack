...
const Checkout: React.FC = () => {
  ...
  const handleQuantityChange = (itemId: number, newQuantity: number) => {
    ...
    if (item && newQuantity > item.stock) {
      setNotificationMessage(
        `Solo hay ${item.stock} unidades disponibles de ${item.marca} ${item.modelo}`
      );
      setNotificationType("error");
      setShowNotification(true);
      return;
  ...
  const handleRemoveItem = (itemId: number) => {
    ...
    setNotificationMessage(
      `${item?.marca} ${item?.modelo} eliminado del carrito`
    );
    setNotificationType("success");
    setShowNotification(true);
  };
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="bg-white rounded-lg shadow-lg p-8 text-center">
            ...
            <button
              ...
              className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition-colors font-semibold rounded-xl"
            >
              Ir a la Tienda
            </button>
          </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-gray-200 text-gray-700 py-3 px-4 rounded-2xl hover:bg-gray-300 transition-colors font-semibold"
                  >
                    ← Volver al Carrito
                  </button>
                </div>
